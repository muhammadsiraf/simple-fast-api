...
def get_kingdom_list(db: Session, skip: int, limit: int):
    response = db_get_kingdom_list(db = db, skip = skip, limit = limit)
    return response
    

def get_kingdom(db: Session, id: int):
    response = db_get_kingdom(db = db, kingdom_id=id)
    return response
    
def create_kingdom(db: Session, req: CreateKingdom):
    request = db_create_kingdom(db = db, req = req)
    return request 


def get_king_list(db: Session, skip: int, limit: int):
    response = db_get_king_list(db = db, skip = skip, limit = limit)
    return response
    

def get_king(db: Session, id: int):
    response = db_get_king(db = db, king_id=id)
    return response
    
    
def create_king(db: Session, req: CreateKing):
    request = db_create_king(db = db, req = req)
    return request
    
    
def get_vizier_list(db: Session, skip:int, limit: int):
    response = db_get_vizier_list(db = db, skip=skip, limit=limit)
    return response
    

def get_vizier(db: Session, id: int):
    response = db_get_vizier(db = db, vizier_id = id)
    return response
    

def create_vizier(db: Session, req: CreateVizier):
    request = db_create_vizier(db = db, req = req)
    return request
    
    
def get_laymen_list(db: Session, skip:int, limit:int):
    response = db_get_laymen_list(db = db, skip=skip, limit=limit)
    return response
    

def get_laymen(db: Session, id: int):
    response = db_get_laymen(db = db, laymen_id = id)
    return response
    

def create_laymen(db: Session, req: CreateLaymen):
